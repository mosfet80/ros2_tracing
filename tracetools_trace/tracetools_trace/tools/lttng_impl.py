...
def is_kernel_paranoid_perf_event(context_fields: Set[str]) -> Tuple[bool, Optional[str]]:
    ...
    """
    print('is_kernel_paranoid_perf_event context_fields=', context_fields)
    ...
    if not has_perf_thread_context_fields:
        return False, None
    path_perf_event_paranoid = '/proc/sys/kernel/perf_event_paranoid'
    paranoid_value_str = Path(path_perf_event_paranoid).read_text().strip()
